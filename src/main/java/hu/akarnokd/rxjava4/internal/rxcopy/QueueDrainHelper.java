...
public final class QueueDrainHelper {
     ...
     * @param downstream the target Subscriber to send events to
        ...
        for (; ; ) {
            ...
            if (state.compareAndSet(r, u)) {
                ...
                if (r == COMPLETED_MASK) {
                    ...
                    postCompleteDrain(n | COMPLETED_MASK, downstream, queue, state, isCancelled);
...
//        }
    ...
    public static <T> void postComplete(Subscriber<? super T> downstream,
        ...
        if (postCompleteDrain(state.get(), downstream, queue, state, isCancelled)) {
            return;
        }
        ...
        for (; ; ) {
            ...
            if (state.compareAndSet(r, u)) {
                ...
                if (r != 0L) {
                    postCompleteDrain(u, downstream, queue, state, isCancelled);
                }
