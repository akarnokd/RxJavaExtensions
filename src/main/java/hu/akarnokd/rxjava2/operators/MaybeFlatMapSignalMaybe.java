 ...
 * @since 0.20.2
 */
final class MaybeFlatMapSignalMaybe<T, R> extends Maybe<R>
implements MaybeTransformer<T, R> {
    ...
    @Override
    public Maybe<R> apply(Maybe<T> t) {
    ...
    implements MaybeObserver<T>, Disposable {
        ...
        public void onComplete() {
            ...
            try {
                next = ObjectHelper.requireNonNull(onCompleteHandler.call(), "The onCompleteHandler returned a null MaybeSource");
            } catch (Throwable ex) {
                Exceptions.throwIfFatal(ex);
                consumer.onError(ex);
                return;
