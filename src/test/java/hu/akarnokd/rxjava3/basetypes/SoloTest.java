...
public class SoloTest implements Consumer<Object>, Action, LongConsumer, Cancellable {
    ...
    public void errorSupplier() {
        Solo.error(new Supplier<Throwable>() {
            @Override
    ...
    @Test
    public void defer() {
        Solo<Integer> sp = Solo.defer(new Supplier<Solo<Integer>>() {
            int i;

            @Override
            public Solo<Integer> get() throws Exception {
                return Solo.just(++i);
            }
        });
    ...
    public void usingEager() {
        Solo.using(Functions.justSupplier(1),
    ...
    public void retryInfinite() {
        ...
        Solo.defer(new Supplier<Solo<Integer>>() {
            @Override
            public Solo<Integer> get() throws Exception {
                if (++counter[0] == 5) {
                    return Solo.just(1);
